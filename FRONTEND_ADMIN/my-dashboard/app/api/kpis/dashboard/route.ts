...
export async function GET() {
  ...
  try {
    const res = await fetch(`${baseUrl}/kpis/dashboard`, { next: { revalidate: 30 } });
    if (!res.ok) {
    ...
    console.error('GET /api/kpis/dashboard failed:', error);
    return NextResponse.json({ error: 'Failed to fetch dashboard KPIs' }, { status: 502 });
  }
}
