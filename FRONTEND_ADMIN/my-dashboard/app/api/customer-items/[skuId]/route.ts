...
export async function GET(
  ...
  try {
    const res = await fetch(`${baseUrl}/customer-items/${encodeURIComponent(skuId)}`, { next: { revalidate: 30 } });
    if (!res.ok) {
    ...
    console.error(`GET /api/customer-items/${skuId} failed:`, error);
    return NextResponse.json({ error: 'Failed to fetch customer item' }, { status: 502 });
  }
}
